...
class OpenCVConan(ConanFile):
  ...
  set(Protobuf_INCLUDE_DIR ${protobuf_INCLUDE_DIR})  # patch: map conan generated v ariable''')
    ...
    def _configure_cmake(self):
        ...
        #mkl has greater priority
        self._cmake.definitions["WITH_LAPACK"] = self.options.with_openblas or self.options.with_mkl
        self._cmake.definitions["WITH_OPENCL"] = self.options.with_opencl
    ...
    def package_info(self):
        ...
        def openblas():
            return ["openblas::openblas"] if self.options.with_openblas else []
        ...
        add_components([
            {"target": "opencv_core",       "lib": "core",       "requires": ["zlib::zlib"] + parallel() + eigen() + openblas()},
            {"target": "opencv_flann",      "lib": "flann",      "requires": ["opencv_core"] + eigen() + openblas()},
            {"target": "opencv_imgproc",    "lib": "imgproc",    "requires": ["opencv_core"] + eigen() + openblas()},
            {"target": "opencv_ml",         "lib": "ml",         "requires": ["opencv_core"] + eigen() + openblas()},
        ...
        if self.settings.os == "Windows":
            self.cpp_info.components["opencv_highgui"].system_libs = ["comctl33", "gdi32", "ole32", "setupapi", "ws2_32", "vfw32"]
