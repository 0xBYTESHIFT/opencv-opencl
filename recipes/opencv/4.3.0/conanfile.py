# This file is from https://conan.io/center/opencv?legacy=true&tab=recipe
# for legacy version 4.3.0
# See also on https://github.com/conan-community/conan-opencv
...
class OpenCVConan(ConanFile):
    ...
    def source(self):
        ...
        tools.get("https://github.com/opencv/opencv/archive/refs/tags/{}.tar.gz".format(self.version), sha256=sha256)
        ...
        tools.get("https://github.com/opencv/opencv_contrib/archive/refs/tags/{}.tar.gz".format(self.version), sha256=sha256)
...
#        os.rename('opencv_contrib-%s' % self.version, 'contrib')
    ...
    def requirements(self):
        self.requires.add('zlib/1.2.11@3opinion/stable')
        if self.options.jpeg:
            ...
            if self.options.jpegturbo:
                self.requires.add('libjpeg-turbo/2.0.4@3opinion/stable')
            else:
                self.requires.add('libjpeg/9d@3opinion/stable')
        if self.options.tiff:
            self.requires.add('libtiff/4.0.9@3opinion/stable')
        if self.options.webp:
            self.requires.add('libwebp/1.0.3@3opinion/stable')
        if self.options.png:
            self.requires.add('libpng/1.6.37@3opinion/stable')
        if self.options.jpeg2000 == "openjpeg":
            self.requires.add('openjpeg/2.3.1@3opinion/stable')
        if self.options.jpeg2000 == "jasper":
            self.requires.add('jasper/2.0.14@3opinion/stable')
        ...
        if not tools.cross_building(self.settings) and self.options.openexr:
            ...
            self.requires.add('openexr/2.3.0@3opinion/stable')
        if self.options.protobuf:
            ...
            self.requires.add('protobuf/3.5.2@3opinion/stable')
        if self.options.eigen:
            self.requires.add('eigen/3.3.7@3opinion/stable')
        if self.options.gstreamer:
            self.requires.add('gstreamer/1.16.0@3opinion/stable')
            self.requires.add('gst-plugins-base/1.16.0@3opinion/stable')
        if self.options.openblas:
            self.requires.add('openblas/0.3.7')
        if self.options.ffmpeg:
            self.requires.add('ffmpeg/4.2.1@3opinion/stable')
        if self.options.lapack:
            self.requires.add('lapack/3.7.1@3opinion/stable')
        if self.options.contrib:
            if self.options.freetype:
                self.requires.add('freetype/2.10.1@3opinion/stable')
            if self.options.harfbuzz:
                self.requires.add('harfbuzz/2.4.0@3opinion/stable')
            if self.options.glog:
                self.requires.add('glog/0.4.0@3opinion/stable')
            if self.options.gflags:
                self.requires.add('gflags/2.2.2@3opinion/stable')
